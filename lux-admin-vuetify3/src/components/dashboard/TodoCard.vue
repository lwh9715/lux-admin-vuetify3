...
<template>
  ...
  <div v-else>
    <v-text-field clearable variant="solo" class="elevation-1 ma-3" hide-details prepend-inner-icon="mdi-magnify" placeholder="Filter Tasks" v-model="searchKey"></v-text-field>
      ...
      <transition-group name="fade">
        <div v-for="todo in filterdTodoList" :key="todo.id">
          <div class="todo-item d-flex align-center pa-5">
            <div>
              <v-checkbox-btn v-model="todo.completed" color="primary" class="pe-2"></v-checkbox-btn>
            </div>
            <v-avatar size="40">
              <v-img src="https://avatars.githubusercontent.com/u/35951244?v=4" alt="alt" />
            </v-avatar>
            <div class="flex-fill mx-5">
              <div class="font-weight-bold" :class="todo.completed ? 'text-decoration-line-through' : ''">
                {{ todo.title }}
              </div>
              ...
              <div>
                <v-chip size="x-small" variant="outlined" class="mr-1 mt-1" :color="getLabelColor(tag)" v-for="tag in todo.tags">
                  {{ tag }}
                </v-chip>
              </div>
            </div>
            <v-btn icon="mdi-delete-outline" variant="text" @click="todoStore.deleteTodoById(todo.id)"></v-btn>
          </div>
        </div>
      </transition-group>
    </perfect-scrollbar>
